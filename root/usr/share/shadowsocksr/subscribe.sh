			...
			if [ -n "$ssr_group" ]; then
				...
				do
					...
					if [ "$subscribe_url_type" = "ss" ]; then
						...
						info=$(urlsafe_b64decode $(echo $temp_info | sed 's/\#.*//') | awk -F '@' '{print $1}')
						temp_info_array=(${info//:/ })
						ssr_type="ss"
						ss_method=${temp_info_array[0]}
						ssr_passwd=${temp_info_array[1]}
						
						info=$(urlsafe_b64decode $(echo $temp_info | sed 's/\#.*//') | awk -F '@' '{print $2}' | awk -F '#' '{print $1}')
						temp_info_array=(${info//:/ })
						ssr_host=${temp_info_array[0]}
						ssr_port=${temp_info_array[1]}
						
						ssr_remarks=$(echo "$temp_info" | awk -F '#' '{print $2}')
				    	fi
